...
**Constraints**: `summary` 없는 게시글은 생성 참조 후보에서 제외, 각 단계는 별도 Convex 함수 단위로 분리, 단계 실패 시 재시도 없이 즉시 중단, 다른 사용자의 데이터 혼입 금지, 최종 글은 한국어 일반 줄글로 반환  
**Scale/Scope**: 인증 사용자 개인용 블로그 생성 기능, 단일 이미지 글 생성과 최대 20장 리뷰 글 생성, 과거 글 요약 기반 RAG, 기존 `convex/*.ts`와 `src/app/(main)` 구조 유지
...
- [x] 입력/업로드 검증 규칙이 정의되었는가? 기존 [`convex/images.ts`](/C:/Users/adcapsule/Desktop/ts-study/blog-agent/convex/images.ts)의 서버 검증을 유지하고, 생성 단계 시작 전 이미지 URL/개수/선택 입력값 검증을 추가한다.
...
- RAG 검색 입력은 원문 `content`가 아니라 `summary`와 `summaryEmbedding`으로 전환한다.
- 생성 액션은 하나의 공개 진입점에서 오케스트레이션하되, 실제 단계는 분리된 internal 함수 호출로 실행한다.
- 실패 응답은 예외 메시지 문자열이 아니라 `failedStage`, `code`, `message`를 포함한 판별 가능한 유니온으로 통일한다.
- 이미지 분석/요약 생성/상위 RAG 구성은 구조화 출력 위주로 수행하고, 최종 글 생성만 `gpt-4o`를 사용한다.

## Phase 1 Design Overview

### Backend / Convex

- `posts` 테이블에 `summary`, `summaryEmbedding`, `summaryStatus`, `summaryError`, `summaryUpdatedAt` 필드를 추가한다.
- 기존 `embedding` 중심 벡터 검색을 `summaryEmbedding` 중심으로 교체하거나 병행 전환하고, 필터는 계속 `userId`를 강제한다.
- 게시글 생성/수정 mutation은 저장 후 `generateSummary` internal action을 스케줄링한다.
- `convex/generate.ts`는 단일 이미지/다중 이미지 플로우 모두에서 `summaryPreparation -> imageAnalysis -> ragContext -> finalDraft` 순서의 내부 스테이지 함수를 호출한다.
- 각 스테이지는 성공 시 구조화된 결과를 반환하고, 실패 시 해당 스테이지 이름과 사용자 노출 가능한 사유를 포함한 실패 객체를 반환한다.

### Frontend / Next.js

- 생성 화면은 성공 결과 타입 외에 단계 실패 응답을 해석할 수 있도록 `ResultData`와 호출 흐름을 확장한다.
- 다중 이미지 리뷰 글 UI는 기존 흐름을 유지하되, 실패 메시지를 스테이지 이름과 함께 표시한다.
- 단일 이미지 생성 플로우가 UI에 없으면 동일 action 계약을 소비하는 최소 진입점 또는 후속 UI 작업을 명시적으로 추가한다.

### Data / Migration

- 기존 게시글은 `summary`가 비어 있으면 새 생성 요청의 참조 후보에서 제외한다.
- 과거 게시글은 백그라운드 재요약 또는 사용자가 수정/재저장할 때 점진적으로 `summary`를 채운다.
